...
export default function LikesPage() {
  ...
  useEffect(() => {
    if (!loading && !user) router.replace("/login?next=/me/likes");
  }, [loading, user, router]);
  useEffect(() => {
    if (!user) return;
    fetchMyReactions("like")
      .then(setItems)
      .catch(() => setItems([]));
  }, [user]);
  ...
  return (
    <main className="mx-auto w-full max-w-7xl flex-1 px-6 py-8">
      <MeSubnav active="/me/likes" />
      <h1 className="mb-4 text-lg font-semibold text-ink">我的点赞</h1>
      {items === null ? (
        <p className="py-20 text-center text-sm text-ink-3">加载中…</p>
      ) : (
        <MyEffectGrid items={items} emptyText="还没有点赞过作品，去广场看看吧" />
      )}
    </main>
  );
}
