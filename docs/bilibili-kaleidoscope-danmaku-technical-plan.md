...
服务端分层：`app/api/*` 只做 HTTP 编排，业务逻辑在 `server/services/`，表读写封装在 `server/repositories/`，连接与实体在 `server/database/`。CLI（`kaleido`）、独立沙箱 Runtime 与内置示例在需要被应用外复用时再抽到 `packages/`；在此之前它们先以 `lib/` 模块形式存在，避免过早拆包。
