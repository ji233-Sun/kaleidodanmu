# Kaleido Danmu（B站万花筒弹幕）原型：技术选型与实现方案
...
flowchart LR
    ...
    Agent["Codex / Coding Agent"] --> CLI["kdanmu CLI"]
    CLI --> Dev["Vite 脚手架"]
...
| 测试 | Vitest（已落地）+ Playwright（待） | 后端单元/集成测试覆盖 utils / Zod schema / repository / service / route 层（`pnpm test`，临时 SQLite 隔离）；Playwright 负责 E2E 与视觉回归（后续阶段） |
| 日志 | Pino | 结构化日志；生产阶段再接 OpenTelemetry |

> Web 端开发服务器由 `next dev` 承担；下面的 Vite 程序化 API 仅用于 **CLI** 的 `kdanmu dev`（启动隔离的表现包预览壳，支持 HMR）和 `kdanmu build`（`build.lib` 产出 ES Module）。Vite 不作为 Web 应用本身的构建链。
...
```bash
kdanmu login
kdanmu init my-effect
kdanmu dev
kdanmu validate
kdanmu build
kdanmu upload --channel draft
```
...
- 先执行 `kdanmu whoami --json`，未认证时调用 `login`。
...
│   ├── repositories/        # 数据访问封装（user / effect / draft / ... .repository.ts）
│   └── services/            # 业务逻辑（auth / effect / version / draft / settings / token .service.ts）
├── lib/
...
├── types/                   # 跨层共享类型：API 请求/响应 Zod schema + DTO（前后端共用）
├── cli/                     # kdanmu CLI 源码（tsup 构建 → dist/cli/index.js，bin: kdanmu）
...
├── tsup.config.ts           # CLI 构建（cli → dist/cli，target node24）
...
**CLI 构建**：`kdanmu` 的源码在 `cli/`，由 tsup（`tsup.config.ts`，target node24）编译为 `dist/cli/index.js`（CJS、注入 shebang），经根 `package.json` 的 `bin` 注册为 `kdanmu` 命令。`pnpm build:cli` 构建；本地用 `pnpm kdanmu <cmd>` 或 `node dist/cli/index.js` 运行；包被安装/发布后 `kdanmu` 命令直接可用。CLI 复用 app 的 tsconfig，但有独立入口与构建、不依赖 Next.js 运行时。独立沙箱 Runtime 与内置示例需要被应用外复用时再抽到 `packages/`。

**API 契约**：请求/响应的 Zod schema 与 DTO 统一放在 `types/`（如 `types/effect.ts`、`types/auth.ts`），服务端校验、前端调用共用同一份类型，避免漂移。
