...
instrumentation.ts         服务端启动时初始化 DataSource
next.config.ts             serverExternalPackages: ['better-sqlite3']
data/                      运行时 SQLite 文件（.gitignore，DB_PATH 默认 ./data/app.db）
```

CLI（`kaleido`）、独立沙箱 Runtime 与内置示例在需要被应用外复用时再抽到 `packages/`；在此之前它们先以 `lib/` 模块形式存在，避免过早拆包。
