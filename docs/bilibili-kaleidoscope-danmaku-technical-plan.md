...
```mermaid
flowchart LR
    Creator["UP主 / 主播"] --> Studio["Creator Studio"]
    ...
    CLI --> Dev["Vite 脚手架"]
    CLI --> Platform["认证 / 上传 API"]
    Platform --> Store["Effect / Artifact"]
    Store --> Runtime
```
...
| 范围 | 选型 | 说明 |
|---|---|---|
| Monorepo | pnpm workspace + TypeScript | 不引入 Nx/Turbo，原型阶段保持简单 |
| Creator Studio | React + TypeScript + Vite 8 | Prompt、对话式创作界面、实时预览、版本切换；代码不对用户暴露 |
...
| 服务端 | Node.js + Fastify + WebSocket | REST、WS Mock、生成、认证和上传统一技术栈 |
| Schema | Zod + JSON Schema | 前后端、CLI、Manifest、消息协议共用类型 |
| 数据 | SQLite + 本地 ArtifactStore | 原型零依赖；生产替换为 PostgreSQL + S3/MinIO |
...
```text
apps/studio        Creator Studio（对话式创作界面 + 预览）
apps/ade           网页 ADE：基于 pi 二开、纯浏览器内运行的 Coding Agent 内核与工具集
apps/api           生成、认证、版本和上传 API
apps/mock-server   点播 REST 与直播 WebSocket
packages/runtime   调度、沙箱、播放器叠加层
packages/adapters  VOD/Live 协议适配
packages/schema    Manifest、消息 Schema
packages/web-fx    沙箱预置的 GSAP / Three.js 锁定版本与类型声明
packages/sdk       defineEffect 与类型
packages/cli       CLI
packages/template  Vite 表现包模板
packages/effects   内置万花筒示例
```
...
生产化之前再接入真实对象存储、PostgreSQL、正式身份系统以及经过授权的 B 站数据源。原型阶段不接入私有接口、不保存用户 Cookie，也不实现协同编辑、公开市场、计费或复杂审核工作流。
